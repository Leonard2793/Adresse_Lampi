...
else if (isset($_POST['make'])) 
{
    ...
    $ort = $_POST['ortname'];
    $insertQuery = 'insert into ort (ort_name) values(?)';
    $existingQuery = 'select * from ort where ort_name = ?';

    try 
    {
        $array = array($ort);
        $stmt = makeStatement($existingQuery, $array);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($row == null) 
        {
            $stmt = makeStatement($insertQuery, $array);
